$(document).ready(function () {
    ...
    if (window.location.pathname.indexOf('orders') !== -1) {
        ...
        $('li#uv').addClass('active');
        cms_paging_order(1);
        cms_order_search();
    }
    ////////////////////////////////////////////////////////
    
    if (window.location.pathname.indexOf('uv') !== -1) {
        $('.input-daterange').datepicker({
            format: "yyyy-mm-dd",
            todayBtn: "linked",
            language: "vi",
...
function cms_func_common() {
    ...
    if (window.location.pathname.indexOf('orders') !== -1) {
        ...
        $("input.discount-uv").keyup(function () {
    ...
    if (window.location.pathname.indexOf('uv') !== -1) {
        ...
        $("input.quantity_product_uv").keyup(function () {
            cms_load_infor_uv();
        });
    }
...
function cms_del_pro_order() {
    $('body').on('click', '.del-pro-uv', function () {
...
function cms_order_search() {
    ...
    $("body").on('keyup', '#uv-search', function (e) {
        if (e.keyCode == 13) {
            cms_paging_order(1);
        }
    });
}
function cms_uv_search() {
    $('#search-option-1').on('change', function () {
        cms_paging_uv(1);
    });
...
function cms_save_orders(type) {
    // if ($('tbody#pro_search_append tr').length == 0) {
    //    $('.ajax-error-ct').html('Xin vui lòng chọn ít nhất 1000 sản phẩm cần xuất trước khi lưu đơn hàng. Xin cảm ơn!').parent().fadeIn().delay(1000).fadeOut('slow');
    // } else {
        $customer_id = $('#search-box-cys').attr('data-id');
        $store_id = $('#store-id').val();
        $date = $('#date-uv').val();
        $note = $('#note-uv').val();
        $note2 = $('#note-order2').val();
        $url = $('#image_file').val();
        $sale_id = $('#sale_id').val();
        $payment_method = $("input:radio[name ='method-pay']:checked").val();
        $discount = 0;//cms_decode_currency_format(typeof $('input.discount-uv').val() == 'undefined' ? 0 : $('input.discount-uv').val());
        $customer_pay = 0;//cms_decode_currency_format($('.customer-pay').val());
        ...
        var $param = {
            ...
            'callback': function (data) {
                if (data == '0') {
                        $('.ajax-error-ct').html('Lỗi rồi cha nội!.').parent().fadeIn().delay(10000).fadeOut('slow');
                        setTimeout(function () {
                        $('.btn-back').delay('10000').trigger('click');
                        }, 1000);
                } else {
                    if (type == 1) {
                        $('.ajax-success-ct').html('Đã lưu thành công đơn hàng 1.').parent().fadeIn().delay(10000).fadeOut('slow');
                        setTimeout(function () {
                            $('.btn-back').delay('10000').trigger('click');
                        }, 1000);
                    } else if (type == 0) {
...
function cms_del_temp_order($id, $page) {
    var conf = confirm('Bạn chắc chắn muốn xóa công việc này?');
    if (conf) {
        var $param = {
            'type': 'POST',
            ...
            'callback': function (data) {
                if (data == '1') {
                    ...
                    $('.ajax-success-ct').html('Xóa công việc thành công !').parent().fadeIn().delay(1000).fadeOut('slow');
                } else if (data == '0') {
                    $('.ajax-error-ct').html('Oops! This system is errors! please try again.').parent().fadeIn().delay(1000).fadeOut('slow');
                }
            }
...
function cms_del_order($id, $page) {
    var conf = confirm('Bạn chắc chắn muốn xóa vĩnh viễn công việc này?');
    if (conf) {
        var $param = {
            'type': 'POST',
            ...
            'callback': function (data) {
                if (data == '1') {
                    cms_paging_order($page);
                    $('.ajax-success-ct').html('Xóa công việc vĩnh viễn thành công.').parent().fadeIn().delay(1000).fadeOut('slow');
                } else if (data == '0') {
                    $('.ajax-error-ct').html('Oops! This system is errors! please try again.').parent().fadeIn().delay(1000).fadeOut('slow');
                }
            }
...
function cms_detail_uv($id) {
    var $param = {
        'type': 'POST',
        'url': 'uv/cms_detail_uv/',
        'data': {'id': $id},
        'callback': function (data) {
            $('.orders').html(data);
        }
...
function cms_show_detail_order($id) {
    $('#tr-detail-uv-' + $id).toggle(200);
    $('.i-detail-uv-' + $id).toggle();
}
function cms_show_detail_uv($id) {
    $('#tr-detail-uv-' + $id).toggle(200);
    $('.i-detail-uv-' + $id).toggle();
}

function cms_show_list_order($id) {
    $('#tr-list-uv-' + $id).toggle(200);
    $('.i-list-uv-' + $id).toggle();
}
function cms_show_list_uv($id) {
    $('#tr-list-uv-' + $id).toggle(200);
    $('.i-list-uv-' + $id).toggle();
}
...
function cms_detail_order_in_customer($id) {
    var $param = {
        ...
        'callback': function (data) {
            ...
            $('#order_info').text('Chi tiết công việc');
        }
    };
    cms_adapter_ajax($param);
}
...
function cms_save_import(type) {
    //if ($('tbody#pro_search_append tr').length == 0) {
     //   $('.ajax-error-ct').html('Xin vui lòng chọn ít nhất 10000 sản phẩm cần xuất trước khi lưu hóa đơn nhập. Xin cảm ơn!').parent().fadeIn().delay(1000).fadeOut('slow');
    //} else {
        $store_id = $('#store-id').val();
        ...
        $detail = [];
        $('tbody#pro_search_append tr').each(function () {
            $price = cms_decode_currency_format($(this).find('input.price-uv').val());
            $id = $(this).attr('data-id');
...
function cms_paging_order_by_customer_id($page) {
    ...
    var $param = {
        ...
        'callback': function (data) {
            $('.orders-main-body').html(data);
            $('#order_info').text('Lịch sử sửa chữa');
        }
    };
    cms_adapter_ajax($param);
}
...
function cms_load_infor_order() {
    ...
    $('tbody#pro_search_append tr').each(function () {
        ...
        $price = $(this).find('td.price-uv-hide').text();
    ...
    if ($('input.discount-uv').val() == '')
        $discount = 0;
    else
        $discount = cms_decode_currency_format($('input.discount-uv').val());

    if ($discount > $total_money) {
        $('input.discount-uv').val($total_money);
