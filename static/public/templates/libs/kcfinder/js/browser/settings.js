...
browser.initSettings = function () {
    ...
    if (!this.orders.length) {
        var orderInputs = $('#uv input[type="radio"]').toArray();
    ...
    $('#uv input[value="' + _.kuki.get('uv') + '"]').get(0).checked = true;
    $('#uv input[name="desc"]').get(0).checked = (_.kuki.get('orderDesc') == 'on');

    $('#uv input[type="radio"]').click(function () {
        _.kuki.set('uv', $(this).get(0).value);
        browser.orderFiles();
    });

    $('#uv input[name="desc"]').click(function () {
