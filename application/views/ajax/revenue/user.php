...
<table class="table table-bordered table-striped">
    ...
    <tr>
        ...
        <th class="text-center">Tổng số công việc</th>
        <!-- <th class="text-center">Tổng chiết khấu</th> -->
        <th class="text-center">Tổng tiền</th>
        <th class="text-center">Tổng VD</th>
        <!-- <th class="text-center">Tổng nợ</th> -->
    </tr>
    </thead>
    <tbody>
    <?php if (isset($_list_users) && count($_list_users)) :
        foreach ($_list_users as $key => $item) :
            ?>
            <tr>
                <td style="text-align: center;">
                    <i style="color: #478fca!important;" title="Chi tiết công việc"
                                                   onclick="cms_show_list_order(<?php echo $item['user_init'];?>)"
                                                   class="fa fa-plus-circle i-list-uv-<?php echo $item['user_init']?>">

                    </i>
                    <i style="color: #478fca!important;" title="Chi tiết công việc"
                       onclick="cms_show_list_order(<?php echo $item['user_init'];?>)"
                       class="fa fa-minus-circle i-hide i-list-uv-<?php echo $item['user_init']?>">
                    </i>
                </td>
            ...
            <tr class="tr-hide" id="tr-list-uv-<?php echo $item['user_init']?>">
                <td colspan="15">
                    <div class="tabbable">
                            ...
                            <div class="tab-pane active">
                                <table class="table table-striped table-bordered table-hover dataTable">
                                    ...
                                    <tr role="row">
                                        ...
                                        <th class="text-left hidden-768">Mã công việc</th>
                                    ...
                                    <?php
                                    $queue= 1;
                                    foreach ($item['_list_orders'] as $uv) {
                                        ?>
                                        <tr>
                                            <td class="text-center width-5 hidden-320 "><?php echo $queue++; ?></td>
                                            <td class="text-center" style="color: #2a6496; cursor: pointer;"
                                                onclick="cms_detail_order(<?php echo $uv['ID']; ?>)"><?php echo $uv['output_code']; ?></td>
                                            <td class="text-center"><?php echo cms_getNamestockbyID($uv['store_id']); ?></td>
                                            <td class="text-center"><?php echo ($uv['sell_date'] != '0000-00-00 00:00:00') ? gmdate("H:i d/m/Y", strtotime(str_replace('-', '/', $uv['sell_date'])) + 7 * 3600) : '-'; ?></td>
                                            <td class="text-center"><?php echo cms_getNameAuthbyID($uv['user_init']); ?></td>
                                            <td class="text-center"><?php echo $uv['total_quantity']; ?></td>
                                            <!-- <td class="text-center"><?php echo cms_encode_currency_format($uv['coupon']); ?></td> -->
                                            <td class="text-center"
                                                style="background-color: #F2F2F2;"><?php echo cms_encode_currency_format($uv['total_money']); ?></td>
                                            <!-- <td class="text-center"
                                                style="background: #fff;"><?php echo cms_encode_currency_format($uv['lack']); ?></td> -->
                                        </tr>
                                    <?php
                                    }
                                    ?>
